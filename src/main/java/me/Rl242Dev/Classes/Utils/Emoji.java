package me.Rl242Dev.Classes.Utils;

public class Emoji {
    private static final String pickaxeEmoji = "<:pickaxe:1107341471725649990>";
    private static final String hoeEmoji = "<:hoe:1109429828454723637>";
    private static final String swordEmoji = "<:sword:1107339351324643388>";
    private static final String totemEmoji = "<:totem_of_undiying:1107341800672346142>";
    private static final String xpEmoji = "<:xp:1111355026338750574>";
    private static final String nametagEmoji = "<:nametag:1111355609095352381>";

    public static String getPickaxeEmoji() {
        return pickaxeEmoji;
    }

    public static String getHoeEmoji() {
        return hoeEmoji;
    }

    public static String getSwordEmoji() {
        return swordEmoji;
    }

    public static String getTotemEmoji() {
        return totemEmoji;
    }

    public static String getXpEmoji() {
        return xpEmoji;
    }

    public static String getNametagEmoji() {
        return nametagEmoji;
    }
}
